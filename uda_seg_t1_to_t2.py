...
from data.data import PancreasCancerDatasetAugmentationFold
...
def train():
    ...
    dset_train = PancreasCancerDatasetAugmentationFold(fold=args.fold)
    ...
    save_arg(args, "t1_to_t2_F%s_%s.txt" % (args.fold, cur_time))
    ...
    for i_iter in range(epochs):
        ...
        if (i_iter + 1) % 1 == 0:
            with open("logs/t1_to_t2_F%s_%s.txt" % (args.fold, cur_time), "a") as f:
                f.write("ave_loss=%s \n" % (np.mean(loss_list)))

    torch.save(enc_shared.state_dict(), join("checkpoints", "t1_to_t2_enc_shared.pth"))
    torch.save(dec_shared.state_dict(), join("checkpoints", "t1_to_t2_dec_shared.pth"))
